...
#define kHostsCornerRadius 5.0f//3.0f

#define kSpacing 5.0f//5.0f
//#define kSpacing 0
...
#define kFixedWidth 320.0f//320.0f
...
@property (nonatomic, copy) void (^cancelButtonPressedBlock)(NSIndexPath *indexPath);
...
- (void)buttonPressed:(JGButton *)button {
    ...
    if (self.cancelButtonPressedBlock) {
        self.cancelButtonPressedBlock([NSIndexPath indexPathForRow:(NSInteger)button.row inSection:self.tag]);
    }
}
...
#pragma clang diagnostic pop
    ...
    for (UIButton *button in self.buttons) {
        ...
        UIImageView *img = [[UIImageView alloc]initWithFrame:CGRectMake(spacing+5+40, height-32, 24, 24)];
        if (i == 1) {
            img.image = [UIImage imageNamed:@"pyq_pre.png"];
            [self addSubview:img];
        }else if (i == 2) {
            img.image = [UIImage imageNamed:@"weixin.png"];
            [self addSubview:img];
        }else if (i == 3) {
            img.image = [UIImage imageNamed:@"xinlang.png"];
            [self addSubview:img];
        }else if (i == 4) {
            img.image = [UIImage imageNamed:@"tengxun.png"];
            [self addSubview:img];
        }else if (i == 5) {
            img.image = [UIImage imageNamed:@"message.png"];
            [self addSubview:img];
        }else if (i == 6) {
            img.image = [UIImage imageNamed:@"friend.png"];
            [self addSubview:img];
        }
...
- (CGRect)layoutForWidth:(CGFloat)width with:(NSInteger)tag{
    CGFloat buttonHeight = 40.0f;
    CGFloat spacing = kSpacing;
...
- (instancetype)initWithSections:(NSArray *)sections {
    ...
    if (self) {
        ...
        void (^cancelPressedBlock)(NSIndexPath *) = ^(NSIndexPath *indexPath) {
            [weakSelf cancelButtonPressed:indexPath];
        };
        ...
        for (JGActionSheetSection *section in self.sections) {
            ...
            if (section.tag == 1) {
                [section setCancelButtonPressedBlock:cancelPressedBlock];
            }else {
...
- (void)cancelButtonPressed:(NSIndexPath *)indexPath {
    if ([self.delegate respondsToSelector:@selector(actionSheet:pressedCancelButtonAtIndexPath:)]) {
        [self.delegate actionSheet:self pressedCancelButtonAtIndexPath:indexPath];
    }
}
...
- (void)layoutSheetForFrame:(CGRect)frame fitToRect:(BOOL)fitToRect initialSetUp:(BOOL)initial continuous:(BOOL)continuous {
    ...
    for (JGActionSheetSection *section in self.sections) {
        ...
        if (section.tag == 1) {
            CGRect f = [section layoutForWidth:width with:section.tag];
